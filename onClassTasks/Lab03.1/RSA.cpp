...
void RSAencrypt(const char *format, const char *publicKeyFile, const char *PlaintextFile, const char *CipherFile)
{
	...
	if (strFormat == "DER")
	{
		cout << "You choose DER - Encryption!" << endl;
		// Load key
		LoadPublicKey(strPublicKeyFile, publicKey);

		// Load plain text from file
		FileSource(PlaintextFile, true, new StringSink(plain), true);
		// cout << "Plain text (from file): " << plain << endl;

		// Init Encryption Function
		RSAES_OAEP_SHA_Encryptor e(publicKey);

		StringSource(plain, true, new PK_EncryptorFilter(rng, e, new FileSink(CipherFile, true)) // PK_EncryptorFilter
		);
	}
	else if (strFormat == "Base64")
	{
...
void RSAdecrypt(const char *format, const char *privateKeyFile, const char *CipherTextFile, const char *PlainTextFile)
{
	...
	if (strFormat == "DER")
	{
		cout << "You choose DER - Decryption!" << endl;
		LoadPrivateKey(privateKeyFile, privateKey);
		// Initialize Decryption Function
		...
		// decryption
		StringSource(cipher, true, new PK_DecryptorFilter(rnd, d, new StringSink(recover)) // PK_DecryptorFilter
		);
		// Pretty print recover text
		// cout << "Recover text: " << recover << endl;
		StringSource(cipher, true, new PK_DecryptorFilter(rnd, d, new FileSink(PlainTextFile, true)) // PK_DecryptorFilter
		);
	}
	else if (strFormat == "Base64")
	{
		cout << "You choose Base64 - Decryption!" << endl;
