...
extern "C"
{
    ...
    DLL_EXPORT const char *RSAencrypt(const char *format, const char *publicKeyFile,
                                      const char *cipherTextFormat,
                                      const char *PlainText);

    DLL_EXPORT const char *RSAdecrypt(const char *format, const char *privateKeyFile,
                                      const char *cipherTextFormat,
...
// Ecrypt
const char *RSAencrypt(const char *format, const char *publicKeyFile,
                       const char *cipherTextFormat,
                       const char *PlainText)
//   const char *CipherText)
{
    ...
    string strFormat(format);
    string strPublicKeyFile(publicKeyFile);
    // string strModePlainText(modePlainText);
    // string strModeCipherText(modeCipherText);
...
const char *RSAdecrypt(const char *format, const char *privateKeyFile,
                       // const char *modeCipherText, const char *modeRecoverText,
                       const char *cipherTextFormat,
                       const char *CipherText)
// , const char *RecoverText)
{
    AutoSeededRandomPool rng;
    string strFormat(format);
    string strPrivateKeyFile(privateKeyFile);
    // string strModeCipherText(modeCipherText);
    // string strModeRecoverText(modeRecoverText);
    string strCipherTextFormat(cipherTextFormat);
    ...
    const char *result = recover.c_str();
    return result;
}
...
// Set locale to support UTF-8
    ...
    if (argc < 2)
    {
        cerr << "Usage: \n"
             ...
             << argv[0] << " encrypt <format> <publicKeyFile> <modePlainText> <modeCipherText> <cipherFormat> <plaintextFile> <cipherFile>\n"
             << argv[0] << " decrypt <format> <privateKeyFile> <modeCipherText> <modeRecoverText> <recoverFormat> <cipherFile> <recoverTextFile>\n"
             << "mode: [fromScreen], [toScreen]\n";
        return -1;
    }
    ...
    {
        cout << RSAencrypt(argv[2], argv[3], argv[4], argv[5]);
    }
    else if (mode == "decrypt")
    {
        cout << RSAdecrypt(argv[2], argv[3], argv[4], argv[5]);
    }
    else
    {
